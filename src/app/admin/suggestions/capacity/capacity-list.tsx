...
import type { CapacityRule } from '@/lib/supabase/types'
import { HourSelect } from '@/components/hour-select'
import { createCapacityRule, deleteCapacityRule } from './actions'

export function CapacityList({ rules }: { rules: CapacityRule[] }) {
  ...
  function handleCreate(formData: FormData) {
    ...
    startTransition(async () => {
      const result = await createCapacityRule(formData)
  ...
  function handleDelete(ruleId: string) {
    setError(null)
    startTransition(async () => {
      const result = await deleteCapacityRule(ruleId)
      if (result.error) setError(result.error)
      router.refresh()
    })
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {rules.length === 0 ? (
        <p className="text-sm text-gray-500">No capacity rules yet — sessions aren&apos;t limited.</p>
      ) : (
        <ul className="flex flex-col divide-y divide-gray-200 rounded-lg border border-gray-200">
          {rules.map((rule) => (
            <li key={rule.id} className="flex items-center justify-between p-3 text-sm">
              <span>
                {rule.start_time.slice(0, 5)}–{rule.end_time.slice(0, 5)}: max {rule.max_concurrent} concurrent
              </span>
              <button
