...
export default async function TherapyNotePage({
  ...
  const { data: session } = await supabase
    .from('session_plans')
    .select('id, student_id, protocol_id, recurrence_type, start_time, day_of_week, status, students(name), protocols(title)')
    .eq('id', sessionId)
    ...
    <main className="mx-auto flex max-w-2xl flex-col gap-6 p-6">
      <div>
        ...
        <p className="text-sm text-gray-500">
          {studentName} — {protocolName} · {dateFormatter.format(new Date(`${sessionDate}T00:00:00Z`))}
        </p>
      </div>
